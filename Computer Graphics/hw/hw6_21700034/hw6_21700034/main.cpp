...
int program, num_of_vertices;
GLuint draw_mode = 0;
GLuint show_wireframe = 0; //You need to change its value according to user's keyboard input.
GLuint show_vertices = 0;
...
void keyboard(unsigned char key, int x, int y)
{
    ...
    switch (key)
    {
    case 'w': show_wireframe = 1; show_vertices = 0; glutPostRedisplay(); break;
    case 'v': show_wireframe = 0; show_vertices = 1; glutPostRedisplay(); break;
    }
}
...
void display()
{
    ...
    T_moon = scale(T_moon, vec3(0.05f));
...
void init()
{
    ...
    get_sphere_3d(vertices, 1.0, 20, 20);
    get_color_3d_by_pos(colors, vertices, 0.0);
    ...
    bind_buffer(vbo[1], colors, program, "vColor", 3);//Make element array buffers to store vertex indices of each face.
...
{
    ShaderInfo shaders[] = {
        {GL_VERTEX_SHADER, "/Users/gwag-yeonghye/hw6_21700034/hw6_21700034/vao_example.vert"},
        {GL_FRAGMENT_SHADER, "/Users/gwag-yeonghye/hw6_21700034/hw6_21700034/vao_example.frag"},
        {GL_NONE, NULL}
    };
